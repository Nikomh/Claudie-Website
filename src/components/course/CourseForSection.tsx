...
import type { CourseForWhomData } from "@/content/courseTypes";

export function CourseForSection({ forWhom }: { forWhom: CourseForWhomData }) {
            ...
            {forWhom.items.map((item, i) => (
              <Reveal key={item.title} delay={i * 80}>
                <div className="flex gap-4 rounded-[var(--radius-md)] border border-border bg-white p-6 shadow-sm">
                  <CheckCircle2
                    className="mt-0.5 h-5 w-5 shrink-0 text-tuerkis"
                    aria-hidden
                  />
